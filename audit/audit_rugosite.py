...
def fetch(lat, lon, model):
    ...
    for attempt in range(5):
        try:
            ...
            if attempt == 4:
                print(f"    ECHEC {model}: {e}", file=sys.stderr)
                return None
        ...
        for (name, lat, lon, zone) in SPOTS:
            ...
            if row:
                ...
                print(f"  {name:28s} elev={row['elevations']} "
                      ...
                      f"{'MIXTE ecart ' + str(row.get('ecart_eau_terre_kn')) if row['mixte'] else 'homogène'}")
            else:
                print(f"  {name:28s} — pas de donnée")
            time.sleep(1.2)
        result[model] = rows
    with open("audit/resultats_rugosite.json", "w") as f:
        json.dump(result, f, indent=1, ensure_ascii=False)
    print("\n→ audit/resultats_rugosite.json")
